elifeLibrary {
    stage 'Checkout', {
        checkout scm
    }

    stage 'Project tests', {
        elifeLocalTests "./project_tests.sh"
        echo 'Checking changes have not been generated by the style checker. If this fails, run .lint.sh from your the venv/ virtualenv'
        sh 'git diff --exit-code'
    }

    stage 'Corpus generation', {
        sh './download-elife-xml.sh'
        sh './generate-article-json.sh > corpus-generation.log 2>&1'
        archive 'corpus-generation.log'
        def generated_green = sh(script: 'grep "^elife-[0-9]\\+-v[0-9]\\+\\.xml ->" corpus-generation.log | grep success', returnStdout: true).trim().toInteger()
        def generated_red = sh(script: 'grep "^elife-[0-9]\\+-v[0-9]\\+\\.xml ->" corpus-generation.log | grep -v success', returnStdout: true).trim().toInteger()
        echo "Generated green,red: ${generated_green}.${generated_red}"
    }

    stage 'Corpus validation', {
        sh './validate-all-json.sh > corpus-validation.log 2>&1'
        archive 'corpus-validation.log'
        def validated_green = sh(script: 'grep "^elife-[0-9]\\+-v[0-9]\\+\\.xml\\.json =>" corpus-generation.log | grep success', returnStdout: true).trim().toInteger()
        def validated_red = sh(script: 'grep "^elife-[0-9]\\+-v[0-9]\\+\\.xml\\.json =>" corpus-generation.log | grep -v success', returnStdout: true).trim().toInteger()
        echo "Validated green,red: ${validated_green}.${validated_red}"
    }
    
    elifeMainlineOnly {    
        stage 'Master', {
            elifeGitMoveToBranch elifeGitRevision(), 'master'
        }
    }
}
